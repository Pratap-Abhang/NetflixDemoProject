...
struct Constants {
    static let API_KEY = "10722d41ee5e05f52f8f1167db3218ed"
    static let BASE_URL = "https://api.themoviedb.org"
    static let YOUTUBE_URL = "https://youtube.googleapis.com/youtube/v3/search?"
    static let YoutubeAPI_Key = "AIzaSyCUMdcEu5B691l-jo72JT65cDc2c3KZOho"
}
...
class APICaller {
    ...
    func getTrendingMovies(completion: @escaping (Result<[Title], Error>) -> Void) {
        ...
        let task = URLSession.shared.dataTask(with: URLRequest(url: url)) { data, _, error in
            ...
            do{
                let results = try JSONDecoder().decode(TredingTitlesResponse.self, from: data)
    ...
    func getDiscoverMovies(completion: @escaping (Result<[Title], Error>) -> Void) {
        guard let url = URL(string: "\(Constants.BASE_URL)/3/discover/movie?api_key=\(Constants.API_KEY)&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&with_watch_monetization_type=flatrate") else {return}
        let task = URLSession.shared.dataTask(with: URLRequest(url: url)) { data, _, error in
            guard let data = data, error == nil else {
                return
            }
    ...
    func getMoviewWithQuery(with query: String, completion: @escaping (Result<VideoElement, Error>) -> Void) {
        guard let query = query.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed) else {return}
        guard let url = URL(string: "\(Constants.YOUTUBE_URL)&q=\(query)&key=\(Constants.YoutubeAPI_Key)") else {return}
        let task = URLSession.shared.dataTask(with: URLRequest(url: url)) { data, _, error in
            guard let data = data, error == nil else {
                return
            }
